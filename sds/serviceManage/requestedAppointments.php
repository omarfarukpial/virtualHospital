...
<table class="table table-striped table-borderd text-center w-auto ml-3 mr-3  table-hover" style="border-radius: 1em;
overflow: hidden;  box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
    ...
    <?php
        ...
        $sql = "SELECT appointmentrequest_id, doctor.name as dname, doctor.specialization as dsp, doctor.fees as dfees,userinfo.name as uname, userinfo.location as uloc, userinfo.phone as uphn, problem,time  FROM appointmentrequest
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<td align=right>". $row["appointmentrequest_id"]. "</td>".
                    ...
                    "<td>". $row["problem"] . "</td>".
                    "<td>". $row["time"] . "</td>".
                    "<td> <a class='btn btn-success' href=acceptAppointmentRequest.php?aid=".$row['appointmentrequest_id']." role='button'>Accept</a> </td>".
                    "<td> <a class='btn btn-danger' href=rejectAppointmentRequest.php?aid=".$row['appointmentrequest_id']." role='button'>Reject</a> </td>"
                    ;
                echo"</tr>";
            }
        } else {
